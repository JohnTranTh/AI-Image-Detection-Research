...
def main():
    ...
    for model in df_real.columns[-args.ix:]:
        ...
        output_rows = []  # store rows to write

        # Ensure we are including image paths
        real_paths = df_real['filename'].tolist()
        fake_paths = df_fake['filename'].tolist()

        real_scores = F.sigmoid(torch.tensor(df_real[model].tolist())).numpy()
        fake_scores = F.sigmoid(torch.tensor(df_fake[model].tolist())).numpy()

        # Process real images
        for path, score in zip(real_paths, real_scores):
            if not math.isnan(score):
                pred_label = int(score >= thresholds[model])
                output_rows.append([path, 0, pred_label, round(float(score), 4)])

        # Process fake images
        for path, score in zip(fake_paths, fake_scores):
            if not math.isnan(score):
                pred_label = int(score >= thresholds[model])
                output_rows.append([path, 1, pred_label, round(float(score), 4)])

        # Write to CSV file for the current model
