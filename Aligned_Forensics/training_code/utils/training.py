...
def add_training_arguments(parser):
    ...
    parser.add_argument(
        "--optim", type=str, default="adamw", help="optim to use [sgd, adam]"
    )
    parser.add_argument(
    ...
    parser.add_argument("--ewc_lambda", type=float, default=None, 
                        help="Strength of EWC regularization (None to disable)")
    return parser
...
class TrainingModel(torch.nn.Module):
    ...
    def __init__(self, opt, subdir='.'):
        ...
        self.ewc_lambda = getattr(opt, "ewc_lambda", None)  # from CLI args
    ...
    def compute_fisher(self, data_loader, samples=200):
        """Estimate Fisher information for parameters using current model."""
        fisher = {}
        ...
        self.model.eval()
        count = 0
        for data in tqdm.tqdm(data_loader, total=min(samples, len(data_loader)), desc="Computing Fisher"):
            img = data['img'].to(self.device)
            label = data['target'].to(self.device).float()

            self.model.zero_grad()
            output, _ = self.model(img, return_feats=False)
    ...
    def train_on_batch(self, data):
        ...
        # EWC penalty
        if self.ewc_lambda is not None and self.ewc_fisher is not None:
            ewc_loss = 0
            for n, p in self.model.named_parameters():
                if n in self.ewc_fisher:
                    ewc_loss += (self.ewc_fisher[n] * (p - self.ewc_params[n]) ** 2).sum()
