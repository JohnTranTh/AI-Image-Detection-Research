...
word_path = '/home/john/ZeroFake/words/most-common-nouns-english.csv'
...
# The pipe already include its scheduler, so there's no point on importing it.
pipe = pipe.to("cuda")
...
for impath in images_list_str:
    ...
    for adv in candidate_prompts:
      ...
      res = cosine_similarity(adv_prompt_word_vector,original_prompt)
      if res < min_sim:
        min_sim = res
        selected = adv
